...
@
\noindent and a log-normal model
<<>>=
m2 = dislnorm$new(moby)
m2$setXmin(m1$getXmin())
m2$setPars(estimate_pars(m2))
@
\noindent In order to compare distributions, $\xmin$ must be equal for both distributions. Plotting the CDFs
...
@
\end{marginfigure}
\noindent suggests that both models perform equally well. The formal hypothesis test
<<>>=
comp = compare_distributions(m1, m2)
@
...
comp$test_statistic
@
\noindent which means we can not reject $H_0$. The $p$-value and test statistic are similar to the values found in table 6.3 of Clauset \textit{et al.}\cite{clauset2009}.
