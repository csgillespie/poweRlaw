...
@
All distribution objects have generic plot methods\footnote{Generic \texttt{lines} and \texttt{points} functions are also available.}
<<fig.keep='none'>>=
...
@
Currently, there is a bug with the \cc{plfit} script when $\xmin = 1$. This is easily demostrated via a simulation 
<<cache=TRUE>>=
x = rpldis(1000, 1, 2)
plfit(x)
@
