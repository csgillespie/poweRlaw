...
%\VignetteIndexEntry{An introduction of the poweRlaw package}
...
@
\noindent For example, the Moby dick data set can be load using\footnote{The package also contains the data set \texttt{moby\_sample}. This data set is two thousand randomly sampled values from the larger \cc{moby} data set.}
<<>>=
data("moby")
@
...
To estimate the lower bound $\xmin$, we minimise the distance between the data and the fitted model CDF, that is
\[
...
and $D(7) = \Sexpr{signif(est$KS, 3)}$. We can then set parameters of power-law
...
\citeauthor{clauset2009} recommend a bootstrap procedure to get a handle on
