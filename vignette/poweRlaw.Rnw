...
The package is hosted on github and\sidenote{The intention is to eventually host this package on CRAN.}  can be installed using the \cc{devtools} package:\sidenote{If use Windows, you will need to install the \texttt{Rtools} package first.}
<<eval=FALSE>>=
