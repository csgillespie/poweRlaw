...
When fitting discrete power-laws two approximations are implemented. 

First, during the bootstrap procedure, generate random numbers. However, when $\alpha$ is close to 1, this can result in very large values. Hence, random numbers over $10^5$ are generated from the continuous power-law.

The second approximation occurs when comparing the data cdf and distribution cdf for discrete power-laws. Again, when $\alpha$ is close 1, data sets with extreme points can be routinely generated. Hence, when comparing distribution, we compare at values $1, 2, 3, \ldots, 10^5$. For value above $10^5$, we generate a sequence from $10^5$ to the largest data point.
