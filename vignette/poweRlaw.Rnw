...
The \texttt{plfit} script also fits continuous power-laws. Again the results are comparable. First we generate one thousand random numbers from the continuous power-law with parameters $\alpha = 2.5$ amd $\xmin = 10$
<<>>=
