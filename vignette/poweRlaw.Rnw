...
The package is hosted on github.\sidenote{The intention is to eventually host this package on CRAN.} The package can be installed using the \cc{devtools} package:\sidenote{If use Windows, you will need to install the \texttt{Rtools} package first.}
<<eval=FALSE>>=
...
Even function and dataset in the package is documented. The command
<<results='hide'>>=
...
@
\noindent Currently, there are two vignettes available. This vignette, which gives an overview of the functionality, and a worked examples vignette. This documents can be accessed via
<<eval=FALSE>>=
...
To fit a discrete power-law,\sidenote{The examples vignette contains a more thorough analysis of this particular data set.} we create a discrete power-law object, \texttt{displ}\sidenote{\cc{displ}: discrete power-law.}
<<>>=
m_m = displ$new(moby)
@
...
\caption{Plot of the data CDF for the Moby Dick data set. This corresponds to figure 6.1(a) in Clauset, 2009. The line corresponds to a power-law distribution with parameters $\xmin=7$ and $\alpha=1.95$.}
\end{marginfigure}
<<>>=
...
\end{itemize}
\noindent The results of the bootstrap can be investigated with histograms
<<fig.keep='none'>>=
...
@
\noindent These commands produce figures \ref{F2} and \ref{F3}.

\clearpage

\subsection{Do we have a power-law?}

Clauset, \textit{el al}, 2009 recommend a bootstrap procedure to determine whether the underlying distribution is a power-law the uncertainty. Essentially, we perform a hypothesis test by generating multiple data sets (with parameters $\xmin$ and $\alpha$) and then "re-inferring" the model parameters. The algorithm is detailed in Algorithm 2.\sidenote{Algorithm 2 can be easily extended for other distributions.}
\begin{table}[t]
\centering
  \begin{tabular}{@{}ll@{}}
    \hline
    ...
    {\small 2:} & Calculate the data KS statistic, $KS_d$.\\
...
\item To calculate the Kolmogorov-Smirnov statistic, we need explore the state space. It is computationally infeasible to explore the entire state space when $\max(x) >> 10^5$. So to make this algorithm computational feasible, we split the state space into two sections. The first section is all values from
\[
...
\]
\end{enumerate}
To estimate the uncertainty with the \cc{moby} data set, we use
...
\end{marginfigure}
\begin{itemize}
\item A $p$-value - \verb|bs$p|. See section 4.2 of the Clauset paper.
...
m_m = displ$new(moby)
@
\noindent The object \cc{m\_m} has class \cc{displ} and inherits the \texttt{discrete\_distribution} class. A list of available distributions is given in table \ref{T1}. 
\begin{table}[h]
  \centering
  \begin{tabular}{@{} lll @{}}
  \toprule
