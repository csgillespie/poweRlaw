...
\noindent which yeilds a lower threshold of $\xmin=\Sexpr{est_ln$xmin}$ and parameters $(\Sexpr{est_ln$pars[1]},$ $\Sexpr{est_ln$pars[2]})$. A similar procedure is applied to fit the Poisson distribution; we create a distribution object using \cc{dispois}, then fit as before.
\begin{marginfigure}
\centering
...
The data CDF and lines of best fit for all three distributions are given figure \ref{F1}. It clear that the Poisson distribution is not apprioate for this data set. However, the log-normal and power-law distribution both provide reasonable fits to the data.
...
To get a handle on the uncertainity in the paramater estimates, we use a bootstrapping procedure, via the \texttt{bootstrap} function. This procedure can be applied to any distribution object. Furthermore, the bootstrap procedure can utilize multiple CPU cores to speed up inference. 
<<eval=FALSE>>=
bs = bootstrap(m_pl, no_of_sims=5000, threads=2)
@
\noindent The boostrap results can be explored in a variety way. First we can estimate the standard deviation of the parameter uncertainity, i.e.
...
A similar bootstrap analysis can be obtained for the lognormal distribution
<<eval=FALSE>>=
bs1 = bootstrap(m_ln)
@
\noindent in this case we would obtain uncertainity estimates for both of the log-normal parameters.
