...
#' \describe{
#' \item{`test_statistic`}{The test statistic.}
#' \item{`p_one_sided`}{A one-sided p-value, which is an upper limit
#' on getting that small a log-likelihood ratio if the
#' first distribution, `d1`, is actually true.}
#' \item{`p_two_sided`}{A two-sided p-value, which is the probability
...
#' \item{`ratio`}{A data frame with two columns. The first column is
#' the `x` value and second column is the difference in
#' log-likelihoods.}}
#' @param d1 A distribution object
#' @param d2 A distribution object
#' @note Code initially based on R code developed by
#' Cosma Rohilla Shalizi (<http://bactra.org/>).
